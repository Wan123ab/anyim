...
public class JianshuService {
    ...
    public JianshuBean parseUserAndBlogs() {
        ...
        try {
            ...
            String api = articlesJsonUri(1);
            JSONArray array = JSONObject.parseArray(doGetJSON(api));
            parseBlog(jianshu, array);
            JSONObject notebooksAndCollections = JSONObject.parseObject(doGetJSON(notebooksAndCollectionsJsonUri(1)));
            parseCollection(jianshu, notebooksAndCollections);
            parseNotebook(jianshu, notebooksAndCollections);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public JianshuBean parseArticleList(int page) {
        ...
        try {
            Document document = doGetHTML(userPageUri());
            parseUser(jianshu, document);
            String api = articlesJsonUri(page);
            JSONArray array = JSONObject.parseArray(doGetJSON(api));
            parseBlog(jianshu, array);
        } catch (IOException e) {
            log.error("请求失败：{}", e.getMessage());
    ...
    private void parseCollection(JianshuBean jianshu, JSONObject collections) {
        Integer page = collections.getInteger("own_collections_page");
        if (page == null){
            page = collections.getInteger("page");
        }
        jianshu.setOwnCollectionPage(page);
        Integer totalPage = collections.getInteger("own_collections_total_pages");
        if (totalPage == null){
            totalPage = collections.getInteger("total_pages");
        }
        jianshu.setOwnCollectionTotalPages(totalPage);
        JSONArray array = collections.getJSONArray("own_collections");
    ...
    private void parseNotebook(JianshuBean jianshu, JSONObject notebooks) {
        Integer page = notebooks.getInteger("notebooks_page");
        if (page == null){
            page = notebooks.getInteger("page");
        }
        jianshu.setNotebookPage(page);
        Integer totalPage = notebooks.getInteger("notebooks_total_pages");
        if (totalPage == null){
            totalPage = notebooks.getInteger("total_pages");
        }
        jianshu.setNotebookTotalPages(totalPage);
    ...
    private void parseBlog(JianshuBean jianshu, JSONArray array) {
        ...
        for (int i = 0; i < array.size(); i++) {
            ...
            article.setCover(object.getString("list_image_url").replace("http:","") + "?imageMogr2/auto-orient/strip%45CimageView2/2/w/1000/format/webp");
     ...
     * 文章列表地址【全部】
     */
    private String articlesJsonUri(int page) {
        return "https://www.jianshu.com/asimov/users/slug/" + config.getUserId() + "/public_notes?page=" + page + "&count=10";
    }
    ...
    private String notebookArticlesJsonUri(String id, int page) {
        return "https://www.jianshu.com/asimov/notebooks/" + id + "/public_notes?page=" + page + "&count=10";
    }
    ...
    private String collectionArticlesJsonUri(String id, int page) {
        return "https://www.jianshu.com/asimov/collections/slug/" + id + "/public_notes?page=" + page + "&count=10&order_by=added_at";
    }

    /**
     * 分类和专题
     */
    private String notebooksAndCollectionsJsonUri(int page) {
        return "https://www.jianshu.com/users/" + config.getUserId() + "/collections_and_notebooks?slug=" + config.getUserId()
